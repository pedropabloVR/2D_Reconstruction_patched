...
public class Main_Sinogram_Reconstruction implements PlugInFilter, DialogListener {
	...
	private int type=1, rot_type=1, offset_type=1, crop_type=1, display_type=1;
	private double lat_tilt_angle=(double) 0.0, s_offset=0.0;
	long start_time;
	short[][][] F;
	short[][][] Fnew;
	...
	public boolean dialogItemChanged(GenericDialog gd, AWTEvent e) {
        ...
        s_offset = (double) gd.getNextNumber();
	...
	// Create 3D array
	void create_3Darray(int type_s, int rot_type, double lat_tilt_angle, int offset_type, double s_offset2, int crop_type, int display_type){
		// set Parameters
		...
		// Parameters
			...
			if (offset_type==2){
				...
				IJ.log("Offset: " +s_offset2 +" pixels");
			} else {
		...
		for (int ss=1;ss<=stack_size;ss++){
			...
			if (rot_type==2){
				...
				if (offset_type==2) {
					ip.translate(s_offset2, 0); //correct for static offset, if 0 nothing is done
				}	
